...
export function useRoomSync(roomId: string): UseRoomSyncReturn {
  ...
  const performAction = useCallback(async (action: RoomAction) => {
    ...
    try {
      // Note: Optimistic updates are now less critical as server updates are faster, 
      // but we keep the structure that relies on server response for final state confirmation.
